...
The MySQL adapter implements the same generic replacement contract transactionally for database-backed repositories. Taxonomy remains on its established JSON-backed path. Test storage mirrors the generic contract so service and repository tests exercise equivalent replacement semantics.
